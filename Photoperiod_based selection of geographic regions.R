...
# Now, let's try the loop again with this implementation.
min_latitude <- 28.07431
max_latitude <- 67.47644
date <- as.Date("2024-03-20") # summer solstice in 2024 = 2024-06-20; spring equinox = 2024-03-20
...
# Adding the last segment if necessary
if(end(regions)$end_latitude < max_latitude) {
  final_day_length <- calculate_day_length(max_latitude, date)
  regions <- rbind(regions, data.frame(start_latitude = start_latitude, end_latitude = max_latitude, day_length_diff = abs(final_day_length - initial_day_length)))
}
...
# Now, updating m_coord_transformed with the correct region_label
m_coord_transformed <- m_coord_transformed %>%
  rowwise() %>%
  mutate(region_label = {
